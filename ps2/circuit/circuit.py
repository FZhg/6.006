...
class PriorityQueue:
    ...
    def __len__(self):
        ...
        return self.queue.heapSize

    def append(self, key):
        """Inserts an element in the priority queue."""
        if key is None:
            raise ValueError('Cannot insert None in the queue')
        self.queue.min_heap_insert(key)
    ...
    def pop(self):
        ...
        if len(self.queue) == 0:
            return None
        return self.queue.extract_heap_min()



class MinHeap:
    """ the heap class for the priority queues """

    def __init__(self):
        self.minHeapList = [None]
        self.heapSize = 0
    ...
    @staticmethod
    def parent(i):
        # get the self.parent index of i
        return i // 2

    @staticmethod
    def left(i):
        # returbnn the left child indexs
        return i * 2
    ...
    def heap_decrease_key(self, i, key):
        # replace the current with a smaller one and find the key's position
        if key > self.minHeapList[i]:
            raise ValueError('New key is bigger than the current key')
        self.minHeapList[i] = key
        while i > 1 and self.minHeapList[self.parent(i)] > self.minHeapList[i]:
            # swap the self.parent with the current key
            tmp = self.minHeapList[self.parent(i)]
            self.minHeapList[self.parent(i)] = self.minHeapList[i]
            self.minHeapList[i] = tmp
            i = self.parent(i)

    def min_heap_insert(self, key):
        #insert a key into the min heap
        self.heapSize = self.heapSize + 1
        self.minHeapList.append(key)
        self.heap_decrease_key(self.heapSize, key)

    def min_heapify(self, i):
        #miantain the min heap property
        l = self.left(i)
        r = self.right(i)

        if l < self.heapSize and self.minHeapList[l] < self.minHeapList[i]:
            smallest = l
        else:
            smallest = i
        if r < self.heapSize and self.minHeapList[r] < self.minHeapList[smallest]:
            smallest = r

        if smallest != i:
            tmp = self.minHeapList[i]
            # swap with the smallest child
            self.minHeapList[i] = self.minHeapList[smallest]
            self.minHeapList[smallest] = tmp
            self.min_heapify(smallest)

    def get_min(self):
        #return the min in the heap
        return self.minHeapList[1]

    def extract_heap_min(self):
        # delete the root
        if self.heapSize < 1:
            raise ValueError('heap overflow')
        min = self.minHeapList[1]
        self.minHeapList[1] = self.minHeapList[self.heapSize]
        self.minHeapList.pop()
        self.heapSize = self.heapSize - 1
        self.min_heapify(1)
        return min

    def get_min_heap(self):
        #return the min heap list
        return self.minHeapList

    def assign_min_heap_list(self, a):
        #a is a list
