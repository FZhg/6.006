...
%%% COLLABORATORS START %%%
I use this website to implement the min heap class:
...
2 \textcolor{red}{Wrong, the fisrt nodes has three nodes , correct answer is 4}
...
2, \textcolor{red}{Wrong, the fisrt nodes has three nodes , correct answer is 0}
%%% PROBLEM 1(i) ANSWER END %%%
}
\newcommand{\answerIj}{ \answer
%%% PROBLEM 1(j) ANSWER START %%%
4, \textcolor{red}{Wrong, the fisrt nodes has three nodes , correct answer is 1}
%%% PROBLEM 1(j) ANSWER END %%%
}
\newcommand{\answerIk}{ \answer
%%% PROBLEM 1(k) ANSWER START %%%
4 \textcolor{red}{Wrong, the fisrt nodes has three nodes , correct answer is 4, by the factor 3}
%%% PROBLEM 1(k) ANSWER END %%%
}
\newcommand{\answerIl}{ \answer
%%% PROBLEM 1(l) ANSWER START %%%
4, \textcolor{red}{Wrong, the fisrt nodes has three nodes , correct answer is 4}
...
1, \textcolor{red}{Correct, but I doubt it!}
%%% PROBLEM 1(t) ANSWER END %%%
}
\newcommand{\answerIu}{ \answer
%%% PROBLEM 1(u) ANSWER START %%%
First, the height of the recursion tree is n  and for level i, $0 \leq i  < n$,  the level has $3 * 4^i$ nodes. For level i , each nodes will  cost rendering time $frac{1}{9^i}$. Thus the level i costs rendering time $3\cdot (\frac{4}{9})$. Adding the series, we can get the total rendering time $\theta(1 - {(\frac{4}{9})}^n) = \theta(1) $. In other words, the Koch Snowflake Patter could never occupy the alll the rectangle area of the phone screen.
...
\answer{
 \_find\_min(), the number 1 in the profile.  
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{figures/profile.png}
\caption{the Running results for the original circuit simulator}
\label{fig:running results}
\end{figure}
}
